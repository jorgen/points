...
points::converter::tree_config_t create_tree_config(uint32_t node_limit, double scale, double offset = -double(uint64_t(1) << 17))
{
  points::converter::tree_config_t tree_config;
  tree_config.node_limit = node_limit;
...
struct tree_test_infrastructure : points::converter::about_to_block_t
{
  tree_test_infrastructure(uint32_t node_limit = 1000)
    : tree_config(create_tree_config(node_limit, 0.001, 0.0))
    , cache_file_error(event_loop, bind(&tree_test_infrastructure::handle_file_error))
    , cache_file_handler("test_cache_file", attributes_config, cache_file_error)
  {
...
TEST_CASE("initialize empty tree", "[converter, tree_t]")
{
  ...
  auto root_id = points::converter::tree_initialize(test_util.tree_config, tree_cache, test_util.cache_file_handler, header, attribute_id, std::move(locations));
...
TEST_CASE("add inclusion", "[converter, tree_t]")
{
  ...
  points::converter::tree_registry_t tree_cache;
  auto write_done_event = create_points(test_util, 0, morton_max, 256);
  auto root_id = points::converter::tree_initialize(test_util.tree_config, tree_cache, test_util.cache_file_handler, write_done_event.header, write_done_event.attribute_id, std::move(write_done_event.locations));

  uint64_t morton_min = ((uint64_t(1) << (1 * 3 * 5 - 1)) - 1);
  auto second_points = create_points(test_util, morton_min, morton_max, 256);

  root_id = points::converter::tree_add_points(test_util.tree_config, tree_cache, test_util.cache_file_handler, root_id, second_points.header, second_points.attribute_id, std::move(second_points.locations));
  auto tree = *tree_cache.get(root_id);
  REQUIRE(tree.morton_max.data[0] == morton_max);
  REQUIRE(tree.morton_max.data[1] == 0);
  REQUIRE(tree.morton_max.data[2] == 0);
...
TEST_CASE("add_new_node", "[converter, tree_t]")
{
  ...
  auto root_id = points::converter::tree_initialize(test_util.tree_config, tree_cache, test_util.cache_file_handler, points.header, points.attribute_id, std::move(points.locations));
...
TEST_CASE("reparent", "[converter, tree_t]")
{
  ...
  points::converter::tree_registry_t tree_cache;
  auto root_id = points::converter::tree_initialize(test_util.tree_config, tree_cache, test_util.cache_file_handler, first_points.header, first_points.attribute_id, std::move(first_points.locations));
  ...
  root_id = points::converter::tree_add_points(test_util.tree_config, tree_cache, test_util.cache_file_handler, root_id, third_points.header, third_points.attribute_id, std::move(third_points.locations));
  {
    auto &tree = *tree_cache.get(root_id);
    REQUIRE(tree.sub_trees.size() == 1);
  }
  ...
  root_id = points::converter::tree_add_points(test_util.tree_config, tree_cache, test_util.cache_file_handler, root_id, fourth_points.header, fourth_points.attribute_id, std::move(fourth_points.locations));
  {
    auto &tree = *tree_cache.get(root_id);
  ...
  root_id = points::converter::tree_add_points(test_util.tree_config, tree_cache, test_util.cache_file_handler, root_id, fifth_points.header, fifth_points.attribute_id, std::move(fifth_points.locations));
  {
    auto &tree = *tree_cache.get(root_id);
    REQUIRE(tree.sub_trees.size() == 2);
