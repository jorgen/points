...
TEST_CASE("deque_map_t functionality")
{
    int last_key = 0;
    ...
    SUBCASE("emplace_back adds values and returns reference")
    {
      auto value_ref = map.emplace_back(test_key_factory(), "test");
      REQUIRE(value_ref == "test");
    }

    SUBCASE("get_value retrieves the correct value for a given key")
    {
      auto opt_value = map.value(1);
      REQUIRE(opt_value == "value1");
    }

    SUBCASE("contains_key returns the correct boolean")
    {
    ...
    SUBCASE("remove successfully removes key-value pair")
    {
    ...
    SUBCASE("Consistent state after multiple emplace_back and remove operations")
    {
