...
// no-ops unless the matching env var (DEW_TEST_S3 / DEW_TEST_AZURE) is set, so a normal run stays
...
//   DEW_TEST_AZURE=1 AZURE_STORAGE_ACCOUNT=devstoreaccount1 AZURE_STORAGE_KEY=<key> \
...
TEST_CASE("S3 live round trip (minio)")
{
  ...
  live_backend_round_trip("s3://" + env_or("DEW_TEST_S3_BUCKET", "pointstest") + "/ds", loop_thread.event_loop());
}

TEST_CASE("Azure live round trip (azurite)")
{
  if (!std::getenv("DEW_TEST_AZURE"))
    return;
  vio::thread_with_event_loop_t loop_thread;
  ...
  std::string container = env_or("DEW_TEST_AZURE_CONTAINER", "pointstest");
  ...
  // Ensure the container exists (PUT container with restype=container). Idempotent (201 or 409).
  {
    ...
    auto rc = run_task(loop, [&]() -> vio::task_t<dew_error_t> {
