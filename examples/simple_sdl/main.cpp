...
template<size_t vertex_size, size_t fragment_size>
int createProgram(const char(&vertex_shader)[vertex_size], const char(&fragment_shader)[fragment_size])
{
  GLuint vs = createShader(vertex_shader, vertex_size, GL_VERTEX_SHADER);
  shader_deleter delete_vs(vs);

  GLuint fs = createShader(fragment_shader, fragment_size, GL_FRAGMENT_SHADER);
...
int main(int , char** )
{
  ...
  GLint attrib_position = glGetAttribLocation(program, "i_position");
  GLint attrib_color = glGetAttribLocation(program, "i_color");
