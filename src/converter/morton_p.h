...
inline void encode(const double (&pos)[3], const double (&scale)[3], morton64_t &morton)
{
  uint64_t ipos[3];
  ipos[0] = uint64_t(pos[0] * scale[0]);
  ipos[1] = uint64_t(pos[1] * scale[1]);
  ipos[2] = uint64_t(pos[2] * scale[2]);
  encode(ipos, morton);
}
