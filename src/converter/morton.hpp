...
inline uint16_t get_name_from_morton_magnitude(int magnitude, const morton192_t &morton)
{
  int lod = magnitude * 5;
  uint16_t ret = 0;
  ret = morton_get_child_mask(lod, morton) | (morton_get_child_mask(lod + 1, morton) << 3 * 1) | (morton_get_child_mask(lod + 2, morton) << 3 * 2) | (morton_get_child_mask(lod + 3, morton) << 3 * 3) |
