...
void input_storage_map_t::add_storage(input_data_id_t id, attributes_id_t attributes_id, std::vector<storage_location_t> &&storage)
{
  ...
  auto &value = _map[id];
  // Re-adding an id (LOD node regenerated by a later pass) abandons its previous blobs: record
  // them so the next checkpoint can free the space (previously they leaked in the file forever).
