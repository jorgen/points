  ...
  , camera_matrix(1)
{
  ...
  callbacks.do_initialize_buffer(color_buffer, type_u8, components_3, int(colors.size() * sizeof(colors[0])), colors.data());
}

void axis_gizmo_data_source_t::rebuild_vertices()
{
  vertices.resize(6);
  glm::vec3 o(0.0f);
...
void axis_gizmo_data_source_t::add_to_frame(const frame_camera_cpp_t &camera, to_render_t *to_render)
{
  ...
  render_list[1].buffer_mapping = axis_gizmo_bm_color;
  render_list[1].user_ptr = color_buffer.user_ptr;
  render_list[2].buffer_mapping = axis_gizmo_bm_camera;
  ...
  draw_group.draw_type = draw_type_t::axis_gizmo_lines;
  draw_group.draw_size = 6;
  to_render_add_render_group(to_render, draw_group);
}

struct axis_gizmo_data_source_t *axis_gizmo_data_source_create(struct renderer_t *renderer, const double center[3], double axis_length)
{
  return new axis_gizmo_data_source_t(renderer->callbacks, glm::dvec3(center[0], center[1], center[2]), axis_length);
}

void axis_gizmo_data_source_destroy(struct axis_gizmo_data_source_t *gizmo)
{
  delete gizmo;
}

struct data_source_t axis_gizmo_data_source_get(struct axis_gizmo_data_source_t *gizmo)
{
  return gizmo->data_source;
}

void axis_gizmo_data_source_set_center(struct axis_gizmo_data_source_t *gizmo, const double center[3])
{
  gizmo->center = glm::dvec3(center[0], center[1], center[2]);
}

void axis_gizmo_data_source_set_axis_length(struct axis_gizmo_data_source_t *gizmo, double axis_length)
{
