...
// Drive a task_t<dew_error_t> to completion on g_loop, single-threaded: pump the loop and yield to
...
template <typename Factory>
dew_error_t drive_blocking(Factory factory)
{
...
bool read_blob_decompressed(object_backend_t *backend, storage_location_t loc, std::shared_ptr<uint8_t[]> &out, uint32_t &out_size)
{
  ...
  dew_error_t err = drive_blocking([backend, loc, raw, br]() { return backend->read_blob(loc, raw->data(), *br); });
  if (err.code != 0)
  {
...
static int dew_open(val config)
{
  g_last_error.clear();
  ...
  dew_error_t err = ds.backend->read_index(ds.index);                   // busy-yields (GET manifest + metadata)
  if (err.code != 0)
  {
...
static std::string dew_last_error()
{
  return g_last_error;
}

static bool dew_exists(int handle)
{
...
static val dew_read_index(int handle)
{
  g_last_error.clear();
  auto it = g_datasets.find(handle);
...
static val dew_read_blob(int handle, double file_id, double offset, double size)
{
  g_last_error.clear();
  auto it = g_datasets.find(handle);
  if (it == g_datasets.end())
  ...
  dew_error_t err = drive_blocking([backend, loc, raw, bytes_read]() { return backend->read_blob(loc, raw->data(), *bytes_read); });
  if (err.code != 0)
  {
    g_last_error = err.msg.empty() ? "read_blob failed" : err.msg;
    return val::null();
...
static val dew_read_node(int handle)
{
  g_last_error.clear();
  auto it = g_datasets.find(handle);
  if (it == g_datasets.end())
...
static void dew_close(int handle)
{
  g_datasets.erase(handle);
}

EMSCRIPTEN_BINDINGS(dew_data)
{
  emscripten::function("open", &dew_open);
  emscripten::function("readIndex", &dew_read_index);
  emscripten::function("readBlob", &dew_read_blob);
  emscripten::function("readNode", &dew_read_node);
  emscripten::function("exists", &dew_exists);
  emscripten::function("lastError", &dew_last_error);
  emscripten::function("close", &dew_close);
}
