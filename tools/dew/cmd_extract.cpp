...
vio::task_t<void> run_extract(vio::event_loop_t &event_loop, extract_args_t args, int &exit_code)
{
  ...
  auto dew_file = std::move(opened_file.value());

  // Read 128-byte index
  uint8_t index_buf[128];
  memset(index_buf, 0, sizeof(index_buf));
  auto read_result = co_await vio::read_file(event_loop, *dew_file, index_buf, 128, 0);
  if (!read_result.has_value() || read_result.value() != 128)
  {
    fmt::print(stderr, "Error: failed to read DEW index\n");
    exit_code = 1;
    event_loop.stop();
    co_return;
  }
  ...
  auto attrib_read = co_await vio::read_file(event_loop, *dew_file, attrib_blob.get(), attrib_configs_loc.size, int64_t(attrib_configs_loc.offset));
  ...
  if (tree_registry_loc.size > 0)
  {
    ...
    auto tree_reg_read = co_await vio::read_file(event_loop, *dew_file, tree_reg_blob.get(), tree_registry_loc.size, int64_t(tree_registry_loc.offset));
  ...
  if (is_tree_mode)
  {
    ...
    else if (args.mode == extract_args_t::mode_t::trees)
    {
      ...
      for (uint32_t ti = 0; ti < uint32_t(tree_registry.locations.size()); ti++)
      {
        ...
        tree_t tree;
        if (!co_await load_tree(event_loop, *dew_file, loc, tree))
        {
  ...
  for (uint32_t ti = 0; ti < uint32_t(tree_registry.locations.size()); ti++)
  {
    ...
    auto tree_read = co_await vio::read_file(event_loop, *dew_file, tree_blob_data.get(), tree_loc.size, int64_t(tree_loc.offset));
  ...
  if (args.write_binary)
  {
    ...
    for (auto &entry : selected)
    {
      ...
      auto rd = co_await vio::read_file(event_loop, *dew_file, buf.get(), entry.location.size, int64_t(entry.location.offset));
